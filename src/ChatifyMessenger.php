...
namespace ChatifyArjuna;

use ChatifyArjuna\Http\Models\Message;
use ChatifyArjuna\Http\Models\Favorite;
use Pusher\Pusher;
...
class ChatifyMessenger
{
    ...
    public function fetchMessage($id){
        ...
        return [
            ...
            'viewType' => ($msg->from_id == Auth::user()->IndexEmployee) ? 'sender' : 'default',
            'seen' => $msg->seen,
        ];
    }
    ...
    public function fetchMessagesQuery($user_id){
        return Message::where('from_id',Auth::user()->IndexEmployee)->where('to_id',$user_id)
                    ->orWhere('from_id',$user_id)->where('to_id',Auth::user()->IndexEmployee);
    }
    ...
    public function makeSeen($user_id){
        Message::Where('from_id',$user_id)
                ->where('to_id',Auth::user()->IndexEmployee)
                ->where('seen',0)
                ->update(['seen' => 1]);
        return 1;
    }
    ...
    public function countUnseenMessages($user_id){
        return Message::where('from_id',$user_id)->where('to_id',Auth::user()->IndexEmployee)->where('seen',0)->count();
    }
    ...
    public function getContactItem($messenger_id, $user){
        // get last message
        $lastMessage = self::getLastMessageQuery($user->IndexEmployee);

        // Get Unseen messages counter
        $unseenCounter = self::countUnseenMessages($user->IndexEmployee);
    ...
    public function inFavorite($user_id){
        return Favorite::where('user_id', Auth::user()->IndexEmployee)
    ...
    public function makeInFavorite($user_id, $action){
        if ($action > 0) {
            ...
            $star->user_id = Auth::user()->IndexEmployee;
            $star->favorite_id = $user_id;
            $star->save();
            return $star ? true : false;
        }else{
            // UnStar
            $star = Favorite::where('user_id',Auth::user()->IndexEmployee)->where('favorite_id',$user_id)->delete();
            return $star ? true : false;
        }
    }
